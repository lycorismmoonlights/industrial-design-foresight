...
来源可在“来源管理”中按推荐来源、RSS、官方 API 或普通网页添加。官方 API 的常用标识通过表单填写；只有维护人员需要时才展开“高级适配器参数”编辑 JSON。新来源默认停用，先测试连接，再确认启用。配置中不得放 API key、token、password 或 secret；这些值必须由运行时环境提供。
